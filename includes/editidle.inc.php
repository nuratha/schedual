...
if (isset($_POST["edit"])) {
    ...
    if (emptyInputQuests($q_name, $q_day, $q_start, $q_end, $q_desc) !== false) {
        ...
        header("location: ../edit_idle.php?qid=".$qid."&error=emptyinput");
        exit();
    }

    //CHECKS TIME CLASH
    if (timeEditClash($con, $id, $qid, $q_day, $q_start, $q_end) !== false) {
        header("location: ../edit_idle.php?qid=".$qid."&error=timeclash");
        exit();
    }

    editIdle($con, $qid, $q_name, $q_day, $q_start, $q_end, $q_desc); // EDITS IDLE

}
else {
    header("location: ../edit_idle.php?qid=".$qid."");
    exit();
}