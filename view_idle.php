<?php
	...
	if(isset($_GET["qid"])) {
		...
		$idle = mysqli_query($con, "SELECT * FROM questlog WHERE user_ID = '$id' AND q_id = '$qid'");
		$iinfo = mysqli_fetch_array($idle);
		mysqli_free_result($idle);
	}
?>
	...
	<!-- BODY -->
		...
		<header class="p-6 flex justify-center space-x-3 bg-primary">
			...
			<div>
				<a href="./edit_idle.php?qid=<?php echo $qid?>">
					<!-- Edit icon -->
			...
			<section class="p-6">
				<div class="p-6 bg-white grid grid-cols-2 gap-4 place-items-center">
					...
					<div>
						<?php
							echo '
							<div class="badge badge-accent text-black">IDLE</div>
							<h2 class="font-serif uppercase text-lg font-semibold text-primary">' . $iinfo['q_name'] . '</h2>
							<h5><b>Day</b>: <div class="badge badge-lg text-white">' . $iinfo['q_day'] . '</div><br>
							<b>Time</b>: ' . substr($iinfo['q_start'], 0, 5) . ' - ' . substr($iinfo['q_end'], 0, 5) . '
							<br><br>
							<b>Description</b>:</h5>
							<p>' . $iinfo['q_desc'] . '</p>';
						?>
					...
					<!-- GRAPHIC -->
					<div>
						<img src="./img/avatar_idle.png" width="350">
					</div>
				</div>
				<hr>
			</section>
